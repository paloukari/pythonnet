...
namespace Python.Runtime
{
    ...
    public class PyList : PySequence
    {
        ...
        public PyList()
        {
            obj = Runtime.Interop.PyList_New(0);
            if (obj == IntPtr.Zero)
            {
        ...
        public PyList(PyObject[] items)
        {
            ...
            obj = Runtime.Interop.PyList_New(count);
            for (var i = 0; i < count; i++)
            {
                IntPtr ptr = items[i].obj;
                Runtime.XIncref(ptr);
                int r = Runtime.Interop.PyList_SetItem(obj, i, ptr);
                if (r < 0)
                {
                    throw new PythonException();
                }
        ...
        {
            IntPtr op = Runtime.Interop.PySequence_List(value.obj);
            if (op == IntPtr.Zero)
            {
                throw new PythonException();
            }
        ...
        {
            int r = Runtime.Interop.PyList_Append(obj, item.obj);
            if (r < 0)
            {
                throw new PythonException();
            }
        ...
        {
            int r = Runtime.Interop.PyList_Insert(obj, index, item.obj);
            if (r < 0)
            {
                throw new PythonException();
            }
        ...
        public void Reverse()
        {
            int r = Runtime.Interop.PyList_Reverse(obj);
            if (r < 0)
            {
                throw new PythonException();
            }
        ...
        public void Sort()
        {
            int r = Runtime.Interop.PyList_Sort(obj);
            if (r < 0)
            {
                throw new PythonException();
            }
