...
namespace Python.Runtime
{
    ...
    public class Dispatcher
    {
        ...
        public object TrueDispatch(ArrayList args)
        {
            MethodInfo method = dtype.GetMethod("Invoke");
            ParameterInfo[] pi = method.GetParameters();
            IntPtr pyargs = Runtime.Interop.PyTuple_New(pi.Length);
            ...
            for (var i = 0; i < pi.Length; i++)
            {
                ...
                Runtime.Interop.PyTuple_SetItem(pyargs, i, arg);
            }

            IntPtr op = Runtime.Interop.PyObject_Call(target, pyargs, IntPtr.Zero);
